export const boardReducer = (boards, action) => {
  switch (action.type) {
    ...
    case "UPDATE_BOARD": {
      const toBeUpdatedBoard = boards.find((board) => board.id === action.payload.id);
    ...
    case "REMOVE_LIST_ID_FROM_BOARD": {
      return boards.map((board) => {
        if (board.id === action.payload.id) {
          board.taskLists = board.taskLists.filter((item) => item.id !== action.payload.taskListId);
        }
        return board
      });
    }
    ...
    case "REMOVE_TASK_ID_FROM_BOARD":{
      return boards.map((board) => {
        if(board.id === action.payload.id){
          board.tasks = board.tasks.filter(item => item.id !== action.payload.taskId);
        }
        return board
      })
    }
